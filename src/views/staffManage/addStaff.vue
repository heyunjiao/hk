<template>
  <div class="setting">
    ...
    <div v-if="!this.formObj.formDisabled" class="btn-line">
      <el-button class="Search-btn" @click="onSubmitFn"
        >{{ $t("useCommonAll.save") }}
      </el-button>
    </div>
  </div>
...
import {GetRoleList,GetPermissionsList} from '@/api/common'
export default {
  components: {
    Form,
    Table,
  ...
  data() {
    return {
      ...
      id: "",
      roleKeyString: "sc_booking,sc_club,sc_club_class,sc_club_room,sc_customer,sc_employee,sc_order,sc_personal,sc_personal,sc_personal_account,sc_personal_info,sc_settlement",
      roleKeys: ['sc_customer', 'sc_booking', 'sc_order', 'sc_personal','sc_personal_info','sc_personal_account'],
      roleArr:[],
      // data: [
      //   {
      //     id: 1,
  ...
  methods: {
    ...
    async getPositionList() {
      ...
      this.positionListArr=result
      this.formObj1.formData[0].options= this.positionListArr
    },
    // 获取所有权限
    async getAllRoleList() {
      const {result} = await GetPermissionsList();
    const removeEnptyData=  this.clearChildrenEmpty(result)
    this.roleArr=removeEnptyData
    },
    clearChildrenEmpty(data) {
      data.forEach((i) => {
        i["disabled"] = true;
        if (i.subItemList) {
          if (i.subItemList.length < 1) {
            delete i.subItemList;
          } else {
            this.clearChildrenEmpty(i.subItemList);
          }
        }
      });
    ...
    ChangeSel(data) {
      ...
      switch (data.value) {
        case 'lobby_staff':
          this.$refs.tree.setCheckedKeys(['sc_customer', 'sc_booking', 'sc_order', 'sc_personal','sc_personal_info','sc_personal_account']);
          break;
        case 'kitchen_staff':
          this.$refs.tree.setCheckedKeys(['sc_booking', 'sc_personal','sc_personal_info','sc_personal_account']);
        ...
        case 'trainer':
          ...
          break;
        case 'treasurer':
          this.$refs.tree.setCheckedKeys(['sc_booking', 'sc_settlement', 'sc_personal','sc_personal_info','sc_personal_account']);

          break;
        case 'manager':
          this.$refs.tree.setCheckedKeys(['sc_customer', 'sc_booking', 'sc_order', 'sc_personal', 'sc_club','sc_club_room', 'sc_club_class','sc_personal_info','sc_personal_account']);
